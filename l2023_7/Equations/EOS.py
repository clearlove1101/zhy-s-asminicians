    ...
    def loop(self, d_p, d_idx, d_rho, d_e, d_cs, d_T):
        ...
        if e < self.us or eta >= 1:
            d_p[d_idx] = (self.a + self.b / (
                    e / self.e0 / eta ** 2 + 1)) * e * rho + self.A * miu + self.B * miu ** 2
        elif e > self.us_:
            z = 1 / eta - 1
            ea = exp(-self.alpha * z)
            eb = exp(-self.beta * z ** 2)
            d_p[d_idx] = self.a * e * rho + (self.b * e * rho / (
                        e / self.e0 / eta ** 2 + 1) + self.B * miu * ea) * eb
        else:
            z = 1 / eta - 1
            ea = exp(-self.alpha * z)
            eb = exp(-self.beta * z ** 2)
            e1 = e - self.us
            pe = self.a * e1 * rho + (self.b * e1 * rho / (
                        e1 / self.e0 / eta ** 2 + 1) + self.A * miu * ea) * eb
            e2 = self.us_ - e
            pc = (self.a + self.b / (
                        e2 / self.e0 / eta ** 2 + 1)) * e2 * rho + self.A * miu + self.B * miu ** 2
            d_p[d_idx] = (pe + pc) / (self.us_ - self.us)
...
class TillotsonEOS_new(Equation):
    """
        最合适的EOS
        但数量级可能还需要根据实际情况调
    """
    ...
    def __init__(self,
        ...
        super(TillotsonEOS_new, self).__init__(dest, sources)
        self.rho0 = rho0
        self.us = us
        self.a = a
        self.b = b
    ...
    def loop(self, d_p, d_idx, d_rho, d_e, d_cs, d_T):
        ...
        d_cs[d_idx] = self.c0 * pow(ratio, 7)
        # d_cs[d_idx] = 1e-4
        d_T[d_idx] = e / self.cv
